...
def compute_loss(net, mem_patch, mem_pos_enc, criterions, labels, conf):
    ...
    for task in conf.tasks.values():
        ...
        for branch_preds in preds:
            pred = branch_preds[t_name].squeeze(-1)
            if t_act == 'softmax':
        ...
        task_loss /= len(preds)  # Average over branches
    ...
    semantic_loss = net.compute_semantic_loss(preds, labels[conf.primary_task])

    # Total loss
    loss += diversity_loss + semantic_loss

    loss /= len(conf.tasks.values())

    return loss, [task_losses, task_preds, task_labels, diversity_loss.item(), semantic_loss.item()]
...
def train_one_epoch(net, criterions, data_loader, optimizer, device, epoch, log_writer, conf):
    ...
    for data_it, data in enumerate(data_loader, start=epoch * len(data_loader)):
        ...
        if batch_full or is_last_batch:
            ...
            task_losses, task_preds, task_labels, diversity_loss, semantic_loss = task_info
