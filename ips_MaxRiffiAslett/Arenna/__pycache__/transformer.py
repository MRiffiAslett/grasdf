...
class MultiHeadCrossAttention(nn.Module):
    ...
    def compute_diversity_loss(self):
        ...
        M, N = self.attn_maps.shape[1], self.attn_maps.shape[2]
        diversity_loss = 0
        for i in range(M):
...
class Transformer(nn.Module):
    ...
    def forward(self, x):
        attn_outputs = [self.mlp(self.crs_attn(x)) for _ in range(self.n_branches)]
        return torch.stack(attn_outputs, dim=1)

    def compute_diversity_loss(self):
        return self.crs_attn.compute_diversity_loss()

    def compute_semantic_loss(self, preds, labels):
        semantic_loss = 0
        for i in range(self.n_branches):
            semantic_loss += F.cross_entropy(preds[:, i], labels)
        return semantic_loss / self.n_branches
