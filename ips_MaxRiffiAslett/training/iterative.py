...
def fill_batch(mem_patch, mem_pos_enc, labels, data, n_prep, n_prep_batch, mem_patch_iter, mem_pos_enc_iter, conf):
    """
...
def compute_loss(net, mem_patch, mem_pos_enc, criterions, labels, conf):
    ...
    diversity_loss = net.compute_diversity_loss()
    loss += diversity_loss

    # Average task losses
    loss /= len(conf.tasks.values())

    return loss, [task_losses, task_preds, task_labels, diversity_loss.item()]
...
def train_one_epoch(net, criterions, data_loader, optimizer, device, epoch, log_writer, conf):
    ...
    for data_it, data in enumerate(data_loader, start=epoch * len(data_loader)):
        ...
        batch_data = fill_batch(mem_patch, mem_pos_enc, labels, data, n_prep, n_prep_batch, mem_patch_iter, mem_pos_enc_iter, conf)
        ...
        if batch_full or is_last_batch:
            ...
            task_losses, task_preds, task_labels, diversity_loss_value = task_info
            ...
            # Update log with additional diversity loss value
            log_writer.update(task_losses, task_preds, task_labels, diversity_loss_value)
...
def evaluate(net, criterions, data_loader, device, log_writer, conf):
    """
    Evaluate the given network on the validation set.
    """
